...
export const DrawPlugin = defineClientPlugin({
  ...
  onInit(context: PluginContext) {
    ...
    drawToggleButton.onclick = () => {
      ...
      if (isCurrentlyDrawing) {
        ...
        context.setState((state) => {
          ...
          if ((state as any).pendingLines && (state as any).pendingLines.length > 0) {
        ...
        debugOverlay.textContent = `Mouse: ${state.lastX},${state.lastY} | Drawing: false`;
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      const state = context.getState() as any;
      debugOverlay.textContent = `Mouse: ${x},${y} | Drawing: ${state.isDrawing}`;
    });
    ...
    document.addEventListener("mousedown", (e) => {
      ...
      context.setState((state) => {
        (state as any).pendingLines = [...((state as any).pendingLines || []), dot];
      });
      ...
      const socket = (window as any).debugSocket;
      if (socket && socket.readyState === WebSocket.OPEN) {
        const initialDrawMessage = {
          type: "draw",
          x,
          y,
          isDrawing: true
        };
        console.log("Sending direct draw message:", JSON.stringify(initialDrawMessage));
        ...
        console.log("Sending initial draw message:", JSON.stringify(initialDrawMessage));
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      context.setState((state) => {
        (state as any).pendingLines = [...((state as any).pendingLines || []), line];
        state.lastX = x;
        state.lastY = y;
      });
      ...
      console.log(`Drew line from (${state.lastX},${state.lastY}) to (${x},${y})`);
    ...
    const stopDrawing = (e: MouseEvent) => {
      ...
      const socket = (window as any).debugSocket;
      if (socket && socket.readyState === WebSocket.OPEN) {
        const stopDrawMessage = {
          type: "draw",
          x: state.lastX,  // Use the last position instead of 0,0
          y: state.lastY,
          isDrawing: false
        };
        console.log("Sending direct draw message:", JSON.stringify(stopDrawMessage));
        ...
        console.log("Sending stop draw message:", JSON.stringify(stopDrawMessage));
  ...
  onMessage(message: ServerMessage, context: PluginContext) {
    ...
    if (message.type === "drawUpdate") {
      ...
      console.log(`Drawing line from (${line.startX},${line.startY}) to (${line.endX},${line.endY}) with color ${line.color}`);
  ...
  onRenderLayer(layerId: string, ctx: CanvasRenderingContext2D, context: PluginContext) {
    ...
    if (layerId === STATIC_LAYER) {
      ...
      console.log(`Rendering ${pendingLines.length} pending lines on active layer`);
